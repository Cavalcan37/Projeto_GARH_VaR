# ETF BR ÍNDICE DE AÇÕES SMALL CAPS 
...
# Passo 2: Obtenção dos dados do ETF Índice de Ações Small Caps (SMAL11.SA) 
smal11.sa <- getSymbols("SMAL11.SA", src = "yahoo", from = "2020-01-01", to = Sys.Date(), auto.assign = FALSE)
smal11.sa <- na.locf(smal11.sa)  # Valores faltantes preenchidos com o último valor válido
# Muinto comum em finanças                           
smal11.sa

# Passo 3: Calcule os retornos do SMAL11.SA
retornos_smal11.sa <- na.omit(ROC(Ad(smal11.sa), type = "discrete"))
retornos_smal11.sa

# Renomeia a coluna
colnames(retornos_smal11.sa) <- "Retornos_SMAL11.SA"
retornos_smal11.sa
...
plot(retornos_smal11.sa, main = "Retornos Diários SMAL11.SA", col = "blue4", lwd = 1)
hist(retornos_smal11.sa, breaks = 25, main = "Distribuição dos Retornos SMAL11.SA", 
...
ArchTest(retornos_smal11.sa, lags = 12)

# Passo 6: Especificação do modelo GARCH(1,1) para SMAL11.SA
especificacao_smal11.sa <- ugarchspec(
  variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
  mean.model = list(armaOrder = c(0, 0), include.mean = TRUE),
  distribution.model = "std"  # Distribuição t de Student
)

# Passo 7: Estimação do modelo GARCH(1,1) para SMAL11.SA
modelo_garch_smal11.sa <- ugarchfit(spec = especificacao_smal11.sa, data = retornos_smal11.sa)
print(modelo_garch_smal11.sa)

# Passo 8: Obtenha a volatilidade estimada para SMAL11.SA
volatilidade_estimada_smal11.sa <- sigma(modelo_garch_smal11.sa)
volatilidade_estimada_smal11.sa
...
plot(volatilidade_estimada_smal11.sa, main = "Volatilidade Estimada SMAL11.SA (GARCH(1,1))", col = "red3", lwd = 2)
legend("topright", 
       legend = c("Volatilidade SMAL11.SA"), 
       col = c("red3"), 
       lwd = 2, 
       cex = 0.8,  # Tamanho da legenda
       bty = "n")  # Remove a caixa ao redor da legenda
...
# Passo 10: Previsão de volatilidade para os próximos 50* dias
previsao_volatilidade <- ugarchforecast(modelo_garch_smal11.sa, n.ahead = 30)
...
df <- modelo_garch_smal11.sa@fit$coef["shape"]  # Graus de liberdade da distribuição t
VaR_historico <- volatilidade_estimada_smal11.sa * qt(alpha, df)

# Plotar o VaR histórico sobre os retornos
plot(retornos_smal11.sa, main = "Retornos SMAL11.SA com VaR (95%)", col = "blue4", lwd = 1)
lines(VaR_historico, col = "red", lwd = 2)
legend("topright", 
       legend = c("Retornos SMAL11.SA", "VaR 95%"), 
       col = c("blue4", "red"), 
       lwd = c(1, 2), 
       cex = 0.8, 
       bty = "n")
...
volatilidade_10_dias <- volatilidade_estimada_smal11.sa * sqrt(10)
...
plot(retornos_smal11.sa, main = "Retornos do SMAL11.SA com VaR (95%) para 10 Dias", col = "blue4", lwd = 1)
lines(VaR_10_dias, col = "red", lwd = 2)
legend("topright", 
       legend = c("Retornos SMAL11.SA", "VaR 95% (10 Dias)"), 
       col = c("blue4", "red"), 
       lwd = c(1, 2), 
       cex = 0.8, 
       bty = "n")
...
volatilidade_diaria <- as.numeric(tail(volatilidade_estimada_smal11.sa, 1))  # Último valor da volatilidade diária
...
# Gráfico GARCH(1,1)
plot(volatilidade_estimada_smal11.sa, main = "Volatilidade SMAL11 (GARCH(1,1))", col = "red3", lwd = 2)
